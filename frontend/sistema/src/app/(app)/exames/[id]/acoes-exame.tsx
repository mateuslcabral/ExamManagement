...
import { excluirExame, restaurarExame } from "../actions";

/** Editar / excluir (lógica, com motivo — C2/P10) / restaurar. */
export function AcoesExame({ exame }: { exame: Exame }) {
  ...
  function executar(fn: () => Promise<{ ok: boolean; erro?: string }>) {
    ...
    iniciar(async () => {
      ...
      setMotivo("");
      router.replace(`/exames/${exame.id}`);
      router.refresh();
    });
  }
  ...
  if (exame.excluido) {
    return (
      <div className="flex flex-col items-end gap-2">
        <Botao variante="secundaria" carregando={pendente} onClick={() => executar(() => restaurarExame(exame.id, exame.pacienteId))}>
          Restaurar exame
        </Botao>
        {erro && (
    ...
    <div className="flex flex-col items-end gap-2">
      ...
      {confirmando && (
        <div className="w-full max-w-sm rounded-xl border border-erro/30 bg-white p-4 shadow-sm">
          <p className="text-sm font-semibold text-texto">Excluir este exame?</p>
          <p className="mt-1 text-xs text-texto-suave">
            Exclusão lógica: sai das telas e dos números, mas fica armazenado com autor, data e motivo.
          </p>
          <label htmlFor="motivo-exclusao-exame" className="mt-3 block text-sm font-medium text-texto">
            Motivo (obrigatório)
          </label>
          <textarea
            id="motivo-exclusao-exame"
            value={motivo}
          ...
          <div className="mt-3 flex justify-end gap-2">
            ...
            <Botao
              ...
              onClick={() => executar(() => excluirExame(exame.id, exame.pacienteId, motivo))}
            >
              Confirmar exclusão
            </Botao>
          </div>
